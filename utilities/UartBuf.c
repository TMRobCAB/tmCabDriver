...
static uint32_t GetBufferCount(volatile uint32_t *pui32Read,
	...
	return ((ui32Write >= ui32Read) ?
			(ui32Write - ui32Read) : (ui32Size - (ui32Read - ui32Write)));
}
#endif
...
int UARTwrite(const char *pcBuf, uint32_t ui32Len) {
	...
	//
	ASSERT(pcBuf != 0);ASSERT(g_ui32Base != 0);
...
int UARTwriteBuf(const char *pcBuf, uint32_t ui32Len) {
#ifdef UART_BUFFERED
	unsigned int uIdx;
...
void UARTmoveRxReadIndex(uint32_t newIndex) {

	g_ui32UARTRxReadIndex += newIndex;
}
...
int UARTreadBuf(char *pcBuf, uint32_t ui32Len) {
#ifdef UART_BUFFERED
	uint32_t ui32Count = 0;
	int8_t cChar;
